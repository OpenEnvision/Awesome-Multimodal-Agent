...
export function selectEntries(catalog, state, saved = new Set()) {
  ...
  const found = catalog.entries.filter(e => {
    ...
    const text = normalize([e.title, e.notes, e.tasks, e.venue, e.collection, ...e.path, ...e.fields.map(f => f.value), ...e.resources.map(r=>r.url)].join(' '));
...
export function fieldHTML(field, catalog) {
  const links=(field.links??[]).map(link=>{
    const anchor=link.url.startsWith('https://github.com/OpenEnvision/Awesome-Multimodal-Agent#')?link.url.split('#')[1]:null;
    const entry=anchor?catalog.entries.find(e=>e.anchor===anchor):null;
    return entry?`<button class="field-link" data-detail="${entry.id}">${esc(link.label)} ↗</button>`:`<a class="field-link" href="${esc(link.url)}" target="_blank" rel="noopener noreferrer">${esc(link.label)} ↗</a>`;
  }).join('');
  return `${esc(field.value)}${links?`<div class="field-links">${links}</div>`:''}`;
}
...
export function rowHTML(e, saved = false) {
  ...
  return `<article class="entry" data-id="${e.id}">
    ...
    <div class="entry-main"><button class="entry-title" data-detail="${e.id}" title="${esc(e.title)}">${esc(shortName)}</button><p>${e.collection === 'systems' ? '<span class="evidence-label">Agent loop · </span>' : ''}${esc(e.notes || e.title)}</p><span class="venue">${esc(e.venue || 'See source for details')}</span></div>
