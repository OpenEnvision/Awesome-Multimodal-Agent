import {PAGE_SIZE, esc, normalize, selectEntries, readState, stateSearch, rowHTML, resourceHTML, fieldHTML, bookmark} from './library.js';
...
function detail(e, trigger = null, push = true) {
  ...
  const loop = e.collection === 'systems' ? e.fields.find(f=>f.label==='Agent loop') : null;
  ...
  $('#detail-content').innerHTML = `<p class="detail-category">${esc(collection.label)} / ${esc(e.path.join(' / '))}</p><h2 id="detail-title">${esc(e.title)}</h2><div class="detail-resources">${e.resources.map(resourceHTML).join('')}</div>${loop ? `<section class="agent-evidence" aria-labelledby="agent-evidence-title"><h3 id="agent-evidence-title">How the agent closes the loop</h3><p>${esc(loop.value)}</p></section>` : ''}<dl class="detail-fields">${e.fields.filter((f,i)=>i>0 && f !== loop && f.label !== 'Links' && f.value && f.value !== '—' && f.value !== '-').map(f=>`<dt>${esc(f.label)}</dt><dd>${fieldHTML(f,catalog)}</dd>`).join('')}</dl>${related.length ? `<div class="related-entries"><p>Also listed in the source</p>${related.map(r=>`<button data-detail="${r.id}">${esc(catalog.collections.find(c=>c.id===r.collection).label)} / ${esc(r.path.at(-1))} ↗</button>`).join('')}</div>` : ''}<div class="detail-actions"><button class="button" data-detail-save="${e.id}" aria-pressed="${saved.has(e.id)}">${saved.has(e.id) ? 'Saved to your reading list' : 'Save to reading list'} ${bookmark}</button><a href="${esc(e.sourceUrl)}" target="_blank" rel="noopener noreferrer">View original README entry ↗</a></div>`;
...
function save(id) {
  ...
  let persisted = true;
  try { localStorage.setItem('openenvision-agents-reading-list-v1', JSON.stringify([...saved])); } catch { persisted = false; }
  render();
...
async function init() {
  try {
    ...
    try {
      const stored = JSON.parse(localStorage.getItem('openenvision-agents-reading-list-v1') || '[]');
    ...
    document.addEventListener('click', event=>{
      ...
      if (target.hasAttribute('data-collection')) {
        ...
        const fromNavigation=target.tagName==='A' && target.getAttribute('href')?.includes('#library');
        event.preventDefault();update({collection:target.dataset.collection,section:'',saved:false},{hash:fromNavigation?'#library':location.hash});
        $('#category-sidebar').classList.remove('mobile-open');$('#mobile-categories').setAttribute('aria-expanded','false');
        if (fromNavigation) { $('#results-title').focus({preventScroll:true}); $('#library').scrollIntoView({behavior:'auto'}); }
      } else if (target.hasAttribute('data-section')) {
        if(target.tagName==='A'&&(event.metaKey||event.ctrlKey||event.shiftKey||event.altKey)) return;
        const section=catalog.sections.find(s=>s.id===target.dataset.section);
        if(!section) return;
        event.preventDefault();
        update({collection:section.collection,section:section.id,saved:false},{hash:target.tagName==='A'?'#library':location.hash});
        $('#category-sidebar').classList.remove('mobile-open');$('#mobile-categories').setAttribute('aria-expanded','false');
        if(target.tagName==='A')$('#library').scrollIntoView({behavior:'auto'});
      }
